...
class NetworkTests(test.BaseAdminViewTests):
    ...
    def test_network_detail_new(self, mac_learning=False):
        ...
        api.neutron.is_extension_supported(
            ...
            'dhcp_agent_scheduler').AndReturn(True)
        self.mox.ReplayAll()
        url = urlunquote(reverse('horizon:network:networks:detail',
                                 args=[network_id]))
    ...
    def _test_network_detail_subnets_tab(self, mac_learning=False):
        ...
        api.neutron.is_extension_supported(
            ...
            'dhcp_agent_scheduler').AndReturn(True)
        self.mox.ReplayAll()
        url = urlunquote(reverse('horizon:network:networks:subnets_tab',
                         args=[network_id]))
        res = self.client.get(url)
    ...
    def test_network_detail_ports_tab(self, mac_learning=False):
        ...
        self.mox.ReplayAll()
        url = reverse('horizon:network:networks:ports_tab',
                      args=[network_id])
    ...
    def test_network_detail_agents_tab(self, mac_learning=False):
        ...
        api.neutron.is_extension_supported(
            ...
            'dhcp_agent_scheduler').AndReturn(True)
        self.mox.ReplayAll()
        url = reverse('horizon:network:networks:agents_tab', args=[network_id])
        res = self.client.get(urlunquote(url))
    ...
    def test_network_create_get(self,
        ...
        self.assertTemplateUsed(res, 'network/networks/create.html')
    ...
    @test.create_stubs({api.neutron: ('is_extension_supported',)})
    @test.update_settings(FILTER_DATA_FIRST={'network.networks': True})
    def test_networks_list_with_admin_filter_first(self):
        api.neutron.is_extension_supported(
            IsA(http.HttpRequest),
            'dhcp_agent_scheduler').AndReturn(True)
        self.mox.ReplayAll()
        res = self.client.get(reverse('horizon:network:networks:index'))
        self.assertTemplateUsed(res, INDEX_TEMPLATE)
        networks = res.context['networks_table'].data
