...
DETAIL_URL = 'horizon:network:networks:ports:detail'

NETWORKS_INDEX_URL = reverse('horizon:network:networks:index')
NETWORKS_DETAIL_URL = 'horizon:network:networks:detail'
...
class NetworkPortTests(test.BaseAdminViewTests):
    ...
    def _test_port_create_get(self, mac_learning=False, binding=False):
        ...
        url = reverse('horizon:network:networks:addport',
                      args=[network.id])
        res = self.client.get(url)

        self.assertTemplateUsed(res, 'network/networks/ports/create.html')
