...
from openstack_dashboard.dashboards.manager.users \
...
class IndexView(tables.DataTableView):
    ...
    template_name = 'manager/users/index.html'
    page_title = _("Users")
    ...
    def get_data(self):
        ...
        if policy.check((("identity", "identity:list_users"),),
            ...
            if filter_first.get('manager.users', False) and len(filters) == 0:
...
class UpdateView(forms.ModalFormView):
    ...
    submit_url = "horizon:manager:users:update"
    success_url = reverse_lazy('horizon:manager:users:index')
    ...
    def get_object(self):
        try:
            ...
            redirect = reverse("horizon:manager:users:index")
            exceptions.handle(self.request,
                              _('Unable to retrieve user information.'),
                              redirect=redirect)
...
class CreateView(forms.ModalFormView):
    ...
    submit_url = reverse_lazy("horizon:manager:users:create")
    success_url = reverse_lazy('horizon:manager:users:index')
...
class DetailView(views.HorizonTemplateView):
    ...
    def get_redirect_url(self):
        return reverse('horizon:manager:users:index')


class ChangePasswordView(forms.ModalFormView):
    template_name = 'manager/users/change_password.html'
    form_id = "change_user_password_form"
    form_class = project_forms.ChangePasswordForm
    submit_url = "horizon:manager:users:change_password"
    submit_label = _("Save")
    success_url = reverse_lazy('horizon:manager:users:index')
